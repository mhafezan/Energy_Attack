...
def Sparsity_Clean_Detection (model, device, offline_sparsity_maps, offline_sparsity_ranges, test_dataset, num_classes, args):
    ...
    wrongly_predicted_as_adversarial_per_class_maps = [0] * num_classes
    wrongly_predicted_as_adversarial_ratio_maps = []

    # Initialization for sparsity-ranges
    num_layers = 20
    layer_inclusion_threshold = 1
    wrongly_predicted_as_adversarial_per_class_ranges = [0] * num_classes
    wrongly_predicted_adversarial_ratio_ranges = []
    ...
    for index, (data, target) in enumerate(tqdm(test_dataset, desc='Data Progress')):
        ...
        for i in range (batch_size):
            ...
            if sum(in_range_status) >= layer_inclusion_threshold:
                # if adversarial == 1:
                wrongly_predicted_as_adversarial_per_class_ranges[single_target.item()] += 1

            ############################################## sparsity-map #######################################################################
            
            sim_rate = jaccard_similarity(offline_sparsity_maps[pred.item()], output[3])

            # if the following condition is True, we predict that the input is adversarial which is a wrong prediction (False Positive)
            if sim_rate <= args.sim_threshold: 
                # if adversarial == 1:
                wrongly_predicted_as_adversarial_per_class_maps[single_target.item()] += 1
    ...
    for predicted, generated in zip(wrongly_predicted_as_adversarial_per_class_maps, generated_clean_for_class):
        wrongly_predicted_as_adversarial_ratio_maps.append((predicted/generated)*100)
    
    for predicted, generated in zip(wrongly_predicted_as_adversarial_per_class_ranges, generated_clean_for_class):
        wrongly_predicted_adversarial_ratio_ranges.append((predicted/generated)*100)
    
    wrongly_predicted_as_adversarial_ratio_maps = ["{:.2f}".format(ratio) for ratio in wrongly_predicted_as_adversarial_ratio_maps]

    wrongly_predicted_adversarial_ratio_ranges = ["{:.2f}".format(ratio) for ratio in wrongly_predicted_adversarial_ratio_ranges]

    overall_error_map = sum(wrongly_predicted_as_adversarial_per_class_maps)/sum(generated_clean_for_class)
    overall_error_range = sum(wrongly_predicted_as_adversarial_per_class_ranges)/sum(generated_clean_for_class)
    
    print(f"\nDistribution of data in each class: {num_of_items_in_class}\n")
    print(f"Wrongly predicted adversarials for each class (map): {wrongly_predicted_as_adversarial_per_class_maps}\n")
    print(f"Wrongly predicted adversarials for each class (rng): {wrongly_predicted_as_adversarial_per_class_ranges}\n")
    print(f"Number of generated clean images for each class: {generated_clean_for_class}\n")
    print(f"Percentage of wrongly predicted adversarials for each class (map): {wrongly_predicted_as_adversarial_ratio_maps}\n")
    print(f"Percentage of wrongly predicted adversarials for each class (rng): {wrongly_predicted_adversarial_ratio_ranges}\n")
...
# python3 resnet18_detect_clean.py --dataset ../../Imagenet_dataset --profile_sparsity_range ../4_profile/profile_data/sparsity_ranges.pt --profile_sparsity_map ../4_profile/profile_data/sparsity_maps.pt --sim_threshold 0.5