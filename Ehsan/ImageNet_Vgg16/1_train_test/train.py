...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ImageNet Training on VGG16')
    ...
    parser.add_argument('--start-epoch', default=0, type=int, metavar='N', help='manual epoch number (useful to restarts)')
    ...
    parser.add_argument('--pin-memory', dest='pin_memory', action='store_true', help='use pin memory')
    ...
    if args.pretrained:
        print("\n=> Using pre-trained model for Training vgg16\n")
        model = vgg16(pretrained=True, args=args, device=device)
    else:
        print("\n=> Training vgg16 model from scratch\n")
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print(f"=> loaded checkpoint '{args.resume}' at epoch {checkpoint['epoch']}")
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint({
            ...
            'best_pred': best_pred,
            'optimizer': optimizer.state_dict()}, is_best, 'checkpoints/vgg16_checkpoint_epoch_' + str(epoch+1) + '.pth')
