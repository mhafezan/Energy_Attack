...
def Sparsity_Attack_Detection(model, device, offline_sparsity_maps, offline_sparsity_ranges, test_dataset, num_classes):
    ...
    for index, (data, target, adversarial) in enumerate(tqdm(test_dataset, desc='Data Progress')): 
        ...
        sim_rate = jaccard_similarity(offline_sparsity_maps[pred.item()], output[12])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', default=7, type=int, help="Achieves a testing accuracy of 85.07% with only 7 epochs") # Newly Added = 100
    parser.add_argument('--batch_size', default=4, type=int, help="Batch size of 4 is a good choice for Training") # Newly Added = 128
    parser.add_argument('--lr', default=0.001, type=float, help="Initial learning rate") # Newly Added = 0.01
    ...
    if args.phase == 'Sparsity-Attack':
        ...
        test_dataset = torch.load('./adversarial_data/adversarial_dataset.pt')
    else:
    ...
    if args.phase == 'train':
        ...
        print()
        print('Accuracy of the network on 2500 test images: %.2f %%' % (100 * correct / total))
...
# Test:    python3 main.py --phase test --weights models/alexnet_cifar_7.pkl
...
# Profile: python3 main.py --phase profile --method sparsity-map/range --weights models/alexnet_cifar_7.pkl
# Detect:  python3 main.py --phase Sparsity-Detect --weights models/alexnet_cifar_7.pkl