...
def get_z_sets_defensegan (modelG, modelD, data, lr, loss, device, rec_iter=500, rec_rr=10, input_latent=100):
    ...
    for idx in range(len(z_hats_recs)):
        ...
        z_hat = torch.randn(data.size(0), input_latent, 1, 1).to(device) # To create a random noise in each iteration of RR
...
def defense_on_clean_image(model, ModelG, ModelD, test_loader, device, args):
    ...
    for batch_idx, (inputs, labels) in enumerate(test_loader):
        ...
        for b in range(z_star.size(0)):
            tmpp = z_star[b]
            z_stars_array.append((tmpp, labels[b]))
        ...
        if batch_idx % display_steps == 0:
            recon_imgs = data_hat[:5] # To pick the first five images as sample
        ...
        running_corrects_dirty += torch.sum(preds_dirty == labels.data)
        num_processed_images += args.batch_size
    
        if batch_idx % display_steps == 0:
            print('# Batch: ', batch_idx+1, '/', len(test_loader), ', Accuracy (reconstructed): ', running_corrects_dirty.item()/num_processed_images, ', Sparsity (dirty): ', (1-(net_ones_reconstruct/net_sizes)),'\n')
        ...
        if batch_idx % display_steps == 0:
            print('# Batch: ', batch_idx+1, '/', len(test_loader), ', Accuracy (clean): ', running_corrects_clean.item() / num_processed_images, ', Sparsity (clean): ', (1-(net_ones_original/net_sizes)),'\n')
    
        del data, data_hat, labels, outputs_dirty, preds_dirty, outputs_clean, preds_clean, z_star
     
    test_acc = running_corrects_dirty.item()/num_processed_images
    
    print('\n', 'rec_iter = ', args.rec_iter, ', rec_rr = ', args.rec_rr, ', accu_reconstruct = ', test_acc, ', sparsity_reconstruct = ', (1-(net_ones_reconstruct/net_sizes)),'\n')
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--dataset', default='../mnist_dataset', help='path to dataset')
    ...
    parser.add_argument('--lr', type=float, default=0.09, help='learning rate to find optimal Z*')
    ...
    parser.add_argument('--im_index_last', type=int, default=60000, help='The last index of the dataset')
    ...
    dataset = dset.MNIST(root=args.dataset, download=True, train=True, transform=transforms.ToTensor())
...
# Arguments: python3 test_clean.py --rec_iter 900 --rec_rr 10 --z_dim 100